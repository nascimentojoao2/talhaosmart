...
function projectCoordinate(longitude: number, latitude: number, originLongitude: number, originLatitude: number): ProjectedPoint {
...
function unprojectCoordinate(point: ProjectedPoint, originLongitude: number, originLatitude: number) {
...
function rotatePoint(point: ProjectedPoint, angleRadians: number): ProjectedPoint {
...
function distanceBetween(a: ProjectedPoint, b: ProjectedPoint) {
  return Math.hypot(a.x - b.x, a.y - b.y);
}

function getPolygonRing(geojson: PolygonGeoJson) {
  ...
  const normalized = ring.map((point) => [Number(point[0]), Number(point[1])] as [number, number]);
...
function getOrigin(ring: [number, number][]) {
...
function getRouteOrientation(projectedRing: ProjectedPoint[]) {
  let longestLength = 0;
  let bestAngle = 0;
...
function buildSweepLevels(minY: number, maxY: number, larguraFaixa: number) {
  const height = maxY - minY;
  if (height <= EPSILON) return { levels: [(minY + maxY) / 2], spacing: larguraFaixa };
...
function getSweepIntervals(rotatedRing: ProjectedPoint[], y: number, sweepIndex: number) {
...
function pushPathPoint(path: ProjectedPoint[], point: ProjectedPoint) {
...
function toLineString(points: ProjectedPoint[], originLongitude: number, originLatitude: number): LineStringGeoJson {
  return {
    type: 'LineString',
...
function buildMissionRoute(rotatedRing: ProjectedPoint[], orientationDegrees: number, originLongitude: number, originLatitude: number, larguraFaixa: number) {
  ...
  const routePath: ProjectedPoint[] = [];
  let currentPoint: ProjectedPoint | null = null;
  ...
  const estatisticas: MissaoOperacionalEstatisticas = {
  ...
  const rota_geojson: GeoJsonFeatureCollection = {
...
export function generateSprayMissionPlan(talhao: TalhaoOperacional, larguraFaixa: number): MissaoOperacionalPlanejada {
  ...
  const { rota_geojson, estatisticas } = buildMissionRoute(
    rotatedRing,
    orientationDegrees,
    origin.longitude,
    origin.latitude,
